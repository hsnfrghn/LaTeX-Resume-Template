...
% Hsn added: SECOND PASS, after the build failed the 2-page limit. The measurement
% Hsn added: that mattered: the sidebar is the TALLER of the two page-1 columns (620pt
% Hsn added: against the body's 581pt), and because CVbody and CVsidebar are minipages
% Hsn added: the whole two-column row is one unbreakable box -- it either fits in what
% Hsn added: is left of page 1 or it moves to page 2 entire. So the block's height is set
% Hsn added: by this file, not by the Experience bullets, and trimming bullets did
% Hsn added: nothing for it. Three cuts here, none of which lose information:
% Hsn added:   - the stray trailing \\ after the last language line, which was adding an
% Hsn added:     empty line to the tallest column;
% Hsn added:   - "(DDD)" and "(RBAC)", where the term was already spelled out in full
% Hsn added:     immediately before the acronym;
% Hsn added:   - Apache (Linux and Docker already carry the deployment story) and
% Hsn added:     "stakeholder communication" (the Sidraw entry opposite evidences it).
% Hsn added: lists pruned. The backend line ran to 14 items, where the last five
% Hsn added: diluted the first five -- a wall of names reads as insecurity. Removed:
% Hsn added: MongoDB (nothing in the work history uses it), Composer (a package manager,
% Hsn added: not a skill -- the same category error as the Postman entry already cut),
% Hsn added: HPC (unsupported anywhere on the document, and it invites a question better
% Hsn added: not taken), and Blade / Alpine.js / Inertia.js (three names for one
% Hsn added: framework family). JWT/OAuth2, DDD and microservices moved down to Systems,
% Hsn added: where they belong alongside SOLID -- they are architecture concepts, not
% Hsn added: backend libraries.
\textbf{Programming Languages:} \\
\vspace{0.1em}
{\small Python, PHP, JavaScript/TypeScript, C\#, SQL} \\
\dotline

% Hsn added: SPRING26-INSTANTES PASS (branch spring26-instantes-jobhunt). Reordered
% Hsn added: only -- no category added, removed or reworded. This posting is a plain
% Hsn added: full-stack role that never mentions AI, so Backend/Frontend Development
% Hsn added: now lead (they carry the JD's own keywords: React, Next.js, Django,
% Hsn added: Laravel, REST API/GraphQL) and AI \& LLM Engineering moves after Systems
% Hsn added: \& Workflow -- still present as a differentiator, just not first.
\textbf{Backend Development:} \\
\vspace{0.1em}
{\small Django, Laravel, Flask, PostgreSQL, MySQL, Redis, REST API/GraphQL, WordPress, WooCommerce} \\
\dotline

\textbf{Frontend Development:} \\
\vspace{0.1em}
{\small React, Next.js, Tailwind, Livewire, jQuery, Bootstrap, Elementor, Accessibility (a11y)} \\
\dotline

\textbf{Systems \& Workflow:} \\
\vspace{0.1em}
{\small Git, Docker, Linux, CI/CD, Caching strategies, SOLID, Domain-Driven Design, Microservices, JWT/OAuth2, Role-Based Access Control} \\
\dotline
...
% Hsn added: new category. The titles on page 1 claim seniority but the sidebar used to
% Hsn added: claim only tooling, so nothing here backed the word "Senior". These are the
% Hsn added: things actually being sold -- scoping work before it is committed, designing
% Hsn added: systems, reviewing other people's code, mentoring, and talking to founders
% Hsn added: and investors -- and each one is evidenced by an Experience entry opposite.
\textbf{Architecture \& Leadership:} \\
\vspace{0.1em}
{\small Technical scoping \& estimation, system design, code review, mentoring} \\
\dotline
